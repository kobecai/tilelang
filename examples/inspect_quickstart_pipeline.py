...
@tilelang.jit
def quickstart_matmul(A, B, block_M: int, block_N: int, block_K: int):
    M, N, K = T.const("M, N, K")
    dtype = T.float16
...
class ArtifactDumper:
    ...
    def dump_manifest(self, config: dict[str, Any]) -> Path:
        lines = [
            "# TileLang Pipeline Artifacts",
            "",
            "This directory was generated by `examples/inspect_quickstart_pipeline.py`.",
            "",
            "For AI-assisted analysis, `all.txt` concatenates the generated artifacts in step order.",
            "",
            "## Config",
            "",
            "```json",
            json.dumps(config, indent=2, sort_keys=True, default=safe_repr),
            "```",
            "",
            "## Files",
            "",
    ...
    def dump_all_text(self, config: dict[str, Any]) -> Path:
        lines = [
            "# TileLang Pipeline Artifacts Combined Dump",
            "",
            "Generated by `examples/inspect_quickstart_pipeline.py`.",
            "",
            "## Config",
            "",
            "```json",
        ...
        for artifact in self.artifacts:
            ...
            try:
                ...
                lines.append(f"<failed to read {artifact.file}: {type(err).__name__}: {err}>\n")
            lines.append("")
...
def dump_ir_generator_artifacts(args: argparse.Namespace, dumper: ArtifactDumper) -> None:
    ...
    jit_entry = spec.jit_entry
    original_source = getattr(jit_entry, "func_source", None)
    if original_source:
        dumper.dump_text(
    ...
    jit_func = getattr(jit_entry, "func", None)
    ir_gen = getattr(jit_func, "ir_gen", None)
    if ir_gen is None:
        dumper.dump_text(
            "ir_generator_unavailable",
            f"This TileLang version does not expose {spec.name}.func.ir_gen, so IRGenerator source cannot be dumped.\n",
    ...
    metadata_lines = [
        ...
        "",
        f"Kernel spec: {spec.name}",
        f"JIT object: {object_summary(jit_entry)}",
        f"JIT mode: {safe_repr(getattr(jit_entry, 'mode', None))}",
        f"Signature: {safe_repr(getattr(jit_entry, 'signature', None))}",
...
def dump_ir_generator_runtime_state(args: argparse.Namespace, dumper: ArtifactDumper) -> None:
    ...
    spec = selected_kernel_spec(args)
    jit_func = getattr(spec.jit_entry, "func", None)
    p1_cache = getattr(jit_func, "p1_cache", None)
    if not p1_cache:
        dumper.dump_text(
            "ir_generator_03_runtime_state",
            f"No phase1 template cache entries were found after {spec.name}.get_tir(...).\n",
    ...
    dumper.dump_text(
        "ir_generator_03_runtime_state",
        "\n".join(lines),
        f"Phase1 TirTemplate cache and constexpr matcher state after {spec.name}.get_tir(...).",
    )
...
def finalize_artifacts(dumper: ArtifactDumper, config: dict[str, Any]) -> None:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Dump step-by-step TileLang compilation artifacts for a selected kernel.",
    )
    parser.add_argument("--kernel", choices=sorted(KERNEL_SPECS), default="issue_2307", help="Kernel spec to inspect.")
...
def main() -> None:
    ...
    try:
        ...
        dumper.dump_text(
            ...
            "Unhandled exception while dumping the pipeline; earlier artifacts are included above this failure record.",
        )
        ...
        raise
    print(f"\nDone. Open {out_dir / 'README.md'} to browse artifacts, or {out_dir / 'all.txt'} for the combined dump.")
