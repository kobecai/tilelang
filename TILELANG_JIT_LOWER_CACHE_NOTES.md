...
- [一条完整调用链](#一条完整调用链)
- [`JITImpl.__call__` 不直接调用 `tilelang.lower`](#jitimpl__call__-不直接调用-tilelanglower)
- [`tilelang.lower` 的输入是什么](#tilelanglower-的输入是什么)
- [lazy 和 eager 两种 JIT 模式](#lazy-和-eager-两种-jit-模式)
  - [lazy style](#lazy-style)
  - [eager style](#eager-style)
- [参数解析和 `JITFunc.p1_cache`](#参数解析和-jitfuncp1_cache)
  - [phase1 key](#phase1-key)
  - [`p1_cache` 的 key/value](#p1_cache-的-keyvalue)
  - [phase2 key](#phase2-key)
- [`JITImpl._kernel_cache`](#jitimpl_kernel_cache)
  - [key](#key)
  - [value](#value)
  - [生命周期](#生命周期)
- [`tilelang.cache.cached` 和 backend cache dispatch](#tilelangcachecached-和-backend-cache-dispatch)
- [`KernelCache._memory_cache`](#kernelcache_memory_cache)
  - [key](#key-1)
  - [value](#value-1)
  - [查找顺序](#查找顺序)
- [cache 访问顺序和短路能力](#cache-访问顺序和短路能力)
- [磁盘 kernel cache](#磁盘-kernel-cache)
  - [目录命名](#目录命名)
  - [kernel cache 的 value 存什么](#kernel-cache-的-value-存什么)
  - [backend 差异](#backend-差异)
  - [从磁盘 value 重建 `JITKernel`](#从磁盘-value-重建-jitkernel)
- [lazy frontend cache](#lazy-frontend-cache)
  - [frontend key](#frontend-key)
  - [frontend value](#frontend-value)
- [`TILELANG_DISABLE_CACHE` 的影响](#tilelang_disable_cache-的影响)
- [线程模型与隔离](#线程模型与隔离)
  - [哪些状态是线程局部的](#哪些状态是线程局部的)
  - [哪些 cache 不是线程局部的](#哪些-cache-不是线程局部的)
  - [GIL 为什么不等于逻辑线程安全](#gil-为什么不等于逻辑线程安全)
  - [`KernelCache._memory_cache` 的锁范围](#kernelcache_memory_cache-的锁范围)
  - [进程模型](#进程模型)
- [磁盘写入的原子性](#磁盘写入的原子性)
- [`tilelang.lower` 内部做什么](#tilelanglower-内部做什么)
- [backend pass pipeline](#backend-pass-pipeline)
- [adapter 如何变成 Python callable](#adapter-如何变成-python-callable)
- [补充：编译期参数、运行期参数和 JIT 编译选项](#补充编译期参数运行期参数和-jit-编译选项)
  - [编译期参数是什么](#编译期参数是什么)
  - [运行期参数是什么](#运行期参数是什么)
  - [p2_key 为什么来自 tensor shape/stride](#p2_key-为什么来自-tensor-shapestride)
  - [JIT 编译选项在哪里](#jit-编译选项在哪里)
  - [三类参数的关系](#三类参数的关系)
- [小结](#小结)
